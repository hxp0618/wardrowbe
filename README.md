...
- `wechat-miniapp/`: WeChat Mini Program client
...
- **Scheduled notifications** - Daily outfit suggestions via ntfy/Mattermost/email/Bark/Expo push plus a generic Webhook channel with presets for Discord, Slack, Telegram, Feishu, and WeCom
...
The WeChat DevTools project points `miniprogramRoot` at `wechat-miniapp/dist/`. That directory is generated by Taro and does not exist until you build the miniapp once.

```bash
# Install workspace dependencies first
npm install

# Use Node.js 20 for the miniapp workspace
nvm use 20

# Generate dist/ once for DevTools import
npm run build:miniapp

# Or keep dist/ updated during development
